...
export const tenants = pgTable(
  'tenants',
  {
    ...
    // Plan
    planTier: text('plan_tier', { enum: ['free', 'growth', 'scale', 'pro', 'enterprise'] }).notNull().default('free'),
    // KMS
  ...
  (t) => ({
    ...
    statusIdx: index('idx_tenants_status').on(t.status).where(sql`status IN ('active','provisioning')`),
  }),
);
