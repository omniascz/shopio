...
export const productVariants = pgTable(
  ...
  (t) => ({
    ...
    skuUnique: uniqueIndex('uq_product_variants_sku').on(t.tenantId, t.sku).where(sql`sku IS NOT NULL`),
    productIdx: index('idx_product_variants_product').on(t.productId, t.position),
    tenantSkuIdx: index('idx_product_variants_tenant_sku').on(t.tenantId, t.sku).where(sql`sku IS NOT NULL`),
  }),
);
