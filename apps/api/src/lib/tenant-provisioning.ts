...
export async function provisionTenantDefaults(tx: DbConn, tenant: TenantSeed): Promise<void> {
  ...
  await tx.insert(schema.shippingRates).values([
    ...
    // Additional carriers (per `14`) — provisioned inactive so the merchant can
    // enable + price the ones they actually use (Nastavení → Doprava). Labels
    // run through the manual carrier (placeholder štítek + ruční tracking).
    {
      tenantId: tenant.id,
      shippingZoneId: zone!.id,
      carrierCode: 'ppl',
      serviceCode: 'home_delivery',
      displayName: 'PPL — doručení domů',
      description: 'Kurýr PPL na adresu.',
      kind: 'flat' as const,
      amount: prices.home,
      currency,
      supportsCod: true,
    ...
    },
    {
      ...
      carrierCode: 'dpd',
      serviceCode: 'home_delivery',
      displayName: 'DPD — doručení domů',
      description: 'Kurýr DPD na adresu.',
      kind: 'flat' as const,
      amount: prices.home,
      currency,
      supportsCod: true,
    ...
    },
    {
      ...
      carrierCode: 'cp',
      serviceCode: 'home_delivery',
      displayName: 'Česká pošta — Balík Do ruky',
      description: 'Doručení Českou poštou na adresu.',
      kind: 'flat' as const,
      amount: prices.home,
      currency,
      supportsCod: true,
